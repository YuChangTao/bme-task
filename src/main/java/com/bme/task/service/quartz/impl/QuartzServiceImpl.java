...
import com.bme.task.job.QuartzMainJobFactory;
import com.bme.task.service.quartz.QuartzService;
import com.bme.task.service.quartz.QuartzTaskErrorsService;
import com.bme.task.service.quartz.QuartzTaskInformationsService;
    ...
    public String runTaskRightNow(String taskNo) {
        ...
        try {
            ...
            if (ResultEnum.HTTP.getMessage().equals(sendType)) {
                try {
                    HttpClientUtil.doPost(url, "application/json", executeParameter);
                    logger.info("");
                } catch (Exception ex) {
                    logger.error("");
